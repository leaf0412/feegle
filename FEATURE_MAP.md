...
> **agent-provider 纯 ACP 架构**:agent 层是 ACP（Agent Client Protocol）客户端,任何 ACP-兼容 agent CLI(claude-agent-acp、codex-acp、gemini、qwen、Kimi、cc-deepseek-via-claude-agent-acp...)都能在 `~/.feegle/config.jsonc` 一行加上去就跑,无需改 feegle 源代码。`kind` 是用户自取的标签,`command` 是要 spawn 的二进制,可选 `args` `env` `cwd` `model` `timeoutMs`。Session 通过 ACP 的 `loadSession` 跨轮恢复(agent 声明该 capability 时),feegle 把 ACP `sessionId` 持久化到 SQLite 的 `sessions` 表(见下方"持久化分层"说明)。

> **repo 绑定与分支解耦**：`/bind_repo <url>` 把仓库绑到当前 scope（群=chatId，单聊=user:&lt;id&gt;，见 `resolveBindingScopeKey`）；未注册的 url 自动注册（不联网探测，占位 `defaultBaseBranch=main`）。`/unbind_repo <url|#|name|id>` 取消单个、`/repo clear` 清空。ChatBinding 只存 `repositoryIds`（branch 已移除）。群聊未绑定仓库时，自然语言 chat 被拦截，回一张交互卡片（url 输入框 + 「绑定仓库」提交按钮，见 `buildBindRepoPromptCard`）；卡片把 scope_key 内嵌进 submit value（卡片回调不带 chat_type，无法重新推导 scope），提交后走 `bind_repo_submit` → `bindRepositoryToScope`（与 `/bind_repo` 共用）并原地更新卡片。卡片另有「取消」按钮（`bind_repo_cancel`，原地更新成「已取消」并 untrack）。**卡片去重/扫尾**：每群可能弹多张提示卡（每条未绑定消息一张），responder 用内存 `Map<scopeKey→Set<msgId>>` 跟踪；任一卡片绑定成功（或群已在别处绑定后下一条消息进来）时，把其余卡片 sweep 成「已失效」并清空跟踪。内存态，重启丢失——重启后遗留卡片再点也无害（同 url 重绑是 DB no-op）。见 `feishu-command-responder.ts` 的 `dispatchChat` / `dispatchBindRepoSubmit` / `dispatchBindRepoCancel` / `sweepBindPrompts`。
